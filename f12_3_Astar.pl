...
astar( Start, Path) :- asserta(count(0)), % ADDED
...
expand( P, l( N, _), _, _, yes, [N|P]) :-
    ...
    goal(N), nl, write("Solution found!"), count(K), nl, write("Count = "), write(K), nl. % NEW
...
update_count(Succ) :-
  ...
  % write('    Expanded count ') , write(C1) , nl, !.
