...
rta(StartNode, [StartNode | Rest])  :-
    retractall( visited( _, _)),	% Retract all previously stored visited nodes
	 asserta(count(0)),           % NEW to keep track of searched nodes
...
rta([ Node])  :- goal( Node), nl, write("Solution found!"), count(K), nl, write("Count = "), write(K), nl. % ADDED.
...
/******************************************************************************/
% NEW predicate for updating count
update_count1(Succ) :-
	% nl , write('Successor list ') , write(Succ),
	retract(count(C)) , length(Succ,LenSucc) ,
	C1 is C + LenSucc , asserta(count(C1)).
	% write('    Expanded count ') , write(C1) , !.
